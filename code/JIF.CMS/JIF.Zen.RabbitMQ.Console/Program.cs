...
namespace JIF.Zen.RabbitMQ.Console
{
    internal class Program
    {
        ...
        {
            System.Console.WriteLine("模式任务执行时长, 单位：ms");
            var delaySeconds = Convert.ToInt32(System.Console.ReadLine());
                ...
                using (var channel = connection.CreateModel())
                {
                    ...
                    consumer.Received += (model, ea) =>
                    {
                        Task.Run(() =>
                        {
                            ...
                            System.Console.WriteLine($"======= 开始处理 {message} 号任务 ========");
...
//                            }
                            ...
                            System.Console.WriteLine($"Recevied: " + message);
                            System.Console.WriteLine($"======= 处理完毕 {message} 号任务 ========");
                        });
                    };

                    channel.BasicConsume(queue: "MQ.MINI.COUPONVERIFICATION.QUEUE",
                        //autoAck: true,
                        autoAck: false, // 关闭 "自动确认模式", 待消费者确定执行完毕之后, 再主动返回确认消息
                        consumer: consumer);

                    System.Console.WriteLine(" Programs is running, Press [enter] to exit.");
                    System.Console.ReadLine();
                }
            }
        }

        private async static Task asyncSimpleQueueConsumerListenerProcess(string message)
        {
            await Task.Run(() =>
            {
                System.Console.WriteLine($"======= 开始处理 {message} 号任务 ========");
        ...
        {
            System.Console.WriteLine("请输入消费者名称: ");
            var consumerName = System.Console.ReadLine();
            ...
            using (var connection = CreateRabbitConnection())
            {
                using (var channel = connection.CreateModel())
                {
                    ...
                    System.Console.WriteLine($" [{consumerName}] Waiting....");
                    ...
                    consumer.Received += (model, ea) =>
                    {
                        ...
                        System.Console.WriteLine($" [{consumerName}] Recevied: {message}");
                    };
        ...
        private static void ExchangesRoutingConsumerListener()
        {
            ...
            System.Console.WriteLine("请输入可接收的日志类型(多个用 , 分隔): ");
            var routingKeys = System.Console.ReadLine();
        ...
        private static void ExchangesTopicsConsumerListener()
        {
            ...
            System.Console.WriteLine("请输入可接收的主题模式(多个用 , 分隔): ");
            var routingKeys = System.Console.ReadLine();
            ...
            using (var connection = CreateRabbitConnection())
            {
                using (var channel = connection.CreateModel())
                {
                    ...
                    consumer.Received += (model, ea) =>
                    {
                        ...
                        System.Console.WriteLine("[{0}] Recevied: {1}", consumerName, message);
                    };
